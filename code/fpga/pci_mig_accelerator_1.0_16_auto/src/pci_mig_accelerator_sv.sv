...
module pci_mig_accelerator_sv #
	(
		...
		parameter integer SYS_ARR_SIZE			= 16,
		parameter integer SYS_MAC_LATENCY		= 20,
...
// Instantiation of Axi Bus Interface M00_AXI
	...
	spad_arbiter #(
		...
		.out_mig_port(arb_mig_port.accel),
	...
	blk_mem_gen_0 bram_a_0 (
		.clka(s00_axi_aclk),
		.addra(bram_a_port_0.addr),
		.dina(bram_a_port_0.bram_din),
		.douta(bram_a_port_0.bram_dout),
		.wea(bram_a_port_0.we)
	);

	blk_mem_gen_0 bram_b_0 (
		.clka(s00_axi_aclk),
		.addra(bram_b_port_0.addr),
		.dina(bram_b_port_0.bram_din),
		.douta(bram_b_port_0.bram_dout),
		.wea(bram_b_port_0.we)
	);

	blk_mem_gen_0 bram_a_1 (
		.clka(s00_axi_aclk),
		.addra(bram_a_port_1.addr),
		.dina(bram_a_port_1.bram_din),
		.douta(bram_a_port_1.bram_dout),
		.wea(bram_a_port_1.we)
	);

	blk_mem_gen_0 bram_b_1 (
		.clka(s00_axi_aclk),
		.addra(bram_b_port_1.addr),
		.dina(bram_b_port_1.bram_din),
		.douta(bram_b_port_1.bram_dout),
		.wea(bram_b_port_1.we)
	);
	...
	logic 	[BRAM_DOUT_WIDTH-1:0]										bram_a_dout_muxout, bram_b_dout_muxout; 
	...
	// ila_0 accel_ila(
	// 	.clk(s00_axi_aclk),
	// 	.probe0	(arb_mig_port.data),
	// 	.probe1	(bram_a_dout_muxout),
	// 	.probe2	(upk_sys_output_o),
	// 	.probe3	(arb_mig_port.addr),
	// 	.probe4	(arb_mig_port.arwlen),
	// 	.probe5	(arb_mig_port.wstrb),

	// 	.probe6	(arb_mig_port.arvalid),
	// 	.probe7	(arb_mig_port.awready),
	// 	.probe8	(arb_mig_port.arready),
	// 	.probe9	(arb_mig_port.wvalid),
	// 	.probe10(arb_mig_port.arwready),
	// 	.probe11(arb_mig_port.awvalid),
	// 	.probe12(arb_mig_port.wlast),
	// 	.probe13(arb_mig_port.wready),
	// 	.probe14(arb_mig_port.bvalid),
	// 	.probe15(arb_mig_port.bready),
	// 	.probe16(arb_mig_port.data_valid),
	// 	.probe17(arb_mig_port.rw_last),

	// 	.probe18(accel_state_o),
	// 	.probe19(acc_ctl_0._sys_read_counter),
	// 	.probe20(acc_ctl_0._mat_mul_m_counter),
	// 	.probe21(acc_ctl_0._mat_mul_count)
	// );
	...
	always_comb begin : unpack_dout
		bram_a_dout_muxout			= bram_rd_sel_o ? bram_a_port_0.bram_dout : bram_a_port_1.bram_dout;
		bram_b_dout_muxout			= bram_rd_sel_o ? bram_b_port_0.bram_dout : bram_b_port_1.bram_dout;

		for (int i = 1; i < (SYS_ARR_SIZE << 1) + 1; i++) begin
			pk_bram_a_dout[i-1]	= bram_a_dout_muxout[i * SYS_ARR_IN_PRECISION - 1 -: SYS_ARR_IN_PRECISION];
			pk_bram_b_dout[i-1]	= bram_b_dout_muxout[i * SYS_ARR_IN_PRECISION - 1 -: SYS_ARR_IN_PRECISION];
		end
	end
	...
	always_comb begin
		arb_mig_port.wready			= m00_axi_wready;
		m00_axi_wdata				= arb_mig_port.wdata;
		m00_axi_wvalid				= arb_mig_port.wvalid;
	end

	always_ff @( s00_axi_aclk ) begin
		arb_mig_port.arready 		<= m00_axi_arready;
		arb_mig_port.awready		<= m00_axi_awready;
		arb_mig_port.rw_last		<= m00_axi_rlast;
		arb_mig_port.data_valid		<= m00_axi_rvalid & m00_axi_rready & |(~m00_axi_rresp);
		arb_mig_port.data			<= m00_axi_rdata;

		// arb_mig_port.wready			<= m00_axi_wready;
		arb_mig_port.bvalid			<= m00_axi_bvalid; 
	end

	always_ff @( m00_axi_aclk ) begin
		m00_axi_araddr				<= arb_mig_port.addr;
		m00_axi_awaddr				<= arb_mig_port.addr;
		m00_axi_arvalid				<= arb_mig_port.arvalid;
		m00_axi_arlen				<= arb_mig_port.arwlen;
		m00_axi_awlen				<= arb_mig_port.arwlen;
		m00_axi_awvalid				<= arb_mig_port.awvalid;

		// w channel
		// m00_axi_wdata				<= arb_mig_port.wdata;
		m00_axi_wstrb				<= arb_mig_port.wstrb;
		m00_axi_wlast				<= arb_mig_port.wlast;
		m00_axi_bready				<= arb_mig_port.bready;
	end
	// User logic ends
