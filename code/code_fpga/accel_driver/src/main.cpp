...
double fpga_gemm(float16_t * mat_a_buf, float16_t * mat_b_buf, float16_t * mat_c_buf, uint32_t fpga_num_mm, uint32_t mat_m_size, bool blocking, bool time) {
	...
	for (int i = 0; i < (fpga_num_mm / mat_m_size); i++) {
		...
		read_to_buffer(READ_DEV_NAME, fpga_r_fd, (char *)mat_c_buf, OUT_MATRIX_BUF_SIZE, mat_c_addr + (i * OUT_MATRIX_BUF_SIZE));
...
int main() {
	...
	torch::Tensor tensor_c			= torch::zeros({2*MATRIX_SIZE, 2*MATRIX_SIZE}, torch::dtype(torch::kFloat16));
	...
	print_imatrix((float16_t *)tensor_c.data_ptr<at::Half>(), 2*MATRIX_SIZE, 2*MATRIX_SIZE, sizeof(float16_t));
