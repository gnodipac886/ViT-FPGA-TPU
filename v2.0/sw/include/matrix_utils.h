...
void set_row_consecutive(float16_t * buf, int M, int N) {
	for (int i = 0; i < M; i++) {
		for (int j = 0; j < N; j++) {
			buf[i * N + j] = (float16_t)(i * (0.01));
		}
	}
}

void set_col_consecutive(float16_t * buf, int M, int N) {
	for (int i = 0; i < M; i++) {
		for (int j = 0; j < N; j++) {
			buf[i * N + j] = (float16_t)(j * (0.01));
		}
	}
}
...
void skew_matrix(void * skew, void * orig, int M, int N, size_t size, int flag=0);
void skew_matrix(void * skew, void * orig, int M, int N, size_t size, int flag) {
	// traverse all the rows
	int skew_N = (N << 1) - (flag ? 0 : 1);
